...
parser.add_argument('-z', '--iterations', type=int, default=30)
...
def model(X, w_h, w_h2, w_o, b_h, b_h2, b_o):
    h = tf.nn.tanh(
        tf.add(
            tf.matmul(X, w_h),
            b_h
...
b_h2 = init_weights([1, hidden_layer])
b_o = init_weights([1, output_layer])

# Prediction
py_x = model(X, w_h, w_h2, w_o, b_h, b_h2, b_o)
